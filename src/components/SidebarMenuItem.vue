  ...
  >
    <template v-if="isRouterLink && !item.disabled">
      ...
      >
        ...
        <template v-if="!item.icon && isFirstLevel && !isMobileItem">
          <img
            class="vsm--icon vsm--icon-placeholder"
            :src="ibcIcon"
          />
        </template>
        <template v-if="(isCollapsed && !isFirstLevel) || !isCollapsed || isMobileItem">
          ...
          >
            <v-icon>{{show ? 'keyboard_arrow_left' : 'keyboard_arrow_down'}}</v-icon>
          </div>
        </template>
      </router-link>
    </template>
...
import { itemMixin, animationMixin } from '../mixins/sidebarItemMixin';
import ibcIcon from '../assets/images/ibc-sp.png'
